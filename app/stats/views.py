        ...
        **kwargs
        ...
        for day_index in range(-6, 4):
            cur_date = datetime.datetime.strptime(
                serialized_date.data["date"], "%Y-%m-%d"
            ) + datetime.timedelta(days=day_index)
            illness_map = {illness: 0 for illness in illnesses}
            for region in regions:
                forecast_result = deepcopy(
                    get_forecast(cur_date.strftime("%Y-%m-%d"), region.pk)
                )
                for illness in list(
                    filter(
                        lambda x: x["name"] in illnesses, forecast_result["illnesses"]
                    )
                ):
                    if mode == "percent":
        ...
        for x in range(len(forecasts)):
            ...
            for y, (illness, value) in enumerate(
                sorted(forecasts[x].items(), key=lambda x: x[0])
            ):
                if mode == "percent":
    ...
    def get(
        ...
        **kwargs
        ...
        for region in regions:
            ...
            forecast_result["illnesses"] = list(
                filter(lambda x: x["name"] in illnesses, forecast_result["illnesses"])
            )
            forecasts.append(forecast_result)
        forecasts.sort(
            key=lambda x: sum(map(lambda y: y["percent"], x["illnesses"])), reverse=True
        )
    ...
    def get(
        ...
        **kwargs
        ...
        for region in regions:
            ...
            forecasts.append(forecast_result)
        min_value = min(
            map(lambda x: sum(map(lambda y: y["percent"], x["illnesses"])), forecasts)
        )
        max_value = max(
            map(lambda x: sum(map(lambda y: y["percent"], x["illnesses"])), forecasts)
        )
        for forecast_ in forecasts:
            forecast_["color"] = get_color_code_by_number(
                sum(map(lambda y: y["percent"], forecast_["illnesses"])),
                max_value,
                min_value,
            )
