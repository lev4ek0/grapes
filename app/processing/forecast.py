...
def forecast(current_date: date, key: int) -> Dict:
    ...
    if len(df_slice[df_slice['WW'].str.contains('ливень|дождь|снег|град|гроза|морось|мгла|туман|морось',
                                                case=False,
                                                na=False)]) == 0:
    ...
    if len(df_slice[df_slice['WW'].str.contains('ливень',
                                                case=False,
                                                na=False)]) > 70:
        showers = True
    else:
    ...
    day_forecast_precipitation = df_slice.groupby(pd.Grouper(key='date', freq='D'))['WW'].apply(
        lambda x: check_rain(x)).reset_index()
    weather_forecast_df = t_and_hum_by_day.iloc[1:].reset_index().merge(day_forecast_precipitation, on=['date'],
                                                                        how='left')
    ...
    if all(t_and_hum_by_day['U'].values >= 85) and all(t_and_hum_by_day['T'].values >= 11):
        mildew_prob = get_prob(t_and_hum_by_day, 23, 100)
    else:
        mildew_prob = 0

    # 2) Оидиум – конидии
    if all(t_and_hum_by_day['T'].values >= 5) and all(t_and_hum_by_day['U'].values >= 55):
    ...
    # 4) Антракноз
    if all(t_and_hum_by_day['T'].values >= 10) and all(t_and_hum_by_day['U'].values >= 60):
        anthracnose_prob = get_prob(t_and_hum_by_day, 27, 75)
    else:
        anthracnose_prob = 0

    # 5) Серая гниль
    if all(t_and_hum_by_day['T'].values >= 12) and all(t_and_hum_by_day['U'].values > 90):
        gray_rot_prob = get_prob(t_and_hum_by_day, 27.5, 100)
    else:
        gray_rot_prob = 0

    # 6) Чёрная пятнистость
    if all(t_and_hum_by_day['T'].values >= 15) and all(t_and_hum_by_day['U'].values > 75):
        black_spot_prob = get_prob(t_and_hum_by_day, 19, 87.5)
    else:
        black_spot_prob = 0

    # 7) Черная гниль
    if all(t_and_hum_by_day['T'].values >= 15) and all(t_and_hum_by_day['U'].values > 90):
        black_rot_prob = get_prob(t_and_hum_by_day, 22.5, 100)
    else:
        black_rot_prob = 0

    # 8) Белая гниль
    if all(t_and_hum_by_day['T'].values >= 14) and all(t_and_hum_by_day['U'].values > 80):
        if showers:
            white_rot_prob = get_prob(t_and_hum_by_day, 23.5, 100, special_condition=True)
        else:
    ...
    if all(t_and_hum_by_day['T'].values >= 18) and all(t_and_hum_by_day['U'].values > 80):
        wilt_prob = get_prob(t_and_hum_by_day, 22.5, 100)
    else:
        wilt_prob = 0

    # 10) Альтернариоз
    if all(t_and_hum_by_day['T'].values >= 11) and all(t_and_hum_by_day['U'].values > 72.5):
        alternariosis_prob = get_prob(t_and_hum_by_day, 24, 87.5)
    else:
        alternariosis_prob = 0

    # 11) Фузариоз
    if all(t_and_hum_by_day['T'].values >= 1) and all(t_and_hum_by_day['U'].values > 40):
        fusarium_prob = get_prob(t_and_hum_by_day, 16.5, 85)
    else:
        fusarium_prob = 0

    # 12) Краснуха
    if all(t_and_hum_by_day['T'].values >= 11) and all(t_and_hum_by_day['U'].values > 80):
        rubella_prob = get_prob(t_and_hum_by_day, 19, 95)
    else:
        rubella_prob = 0

    # 13) Бактериальный рак
    if all(t_and_hum_by_day['T'].values >= 17) and all(t_and_hum_by_day['U'].values > 80):
        bacterial_cancer_prob = get_prob(t_and_hum_by_day, 27.5, 95)
    else:
        bacterial_cancer_prob = 0

    return {'date': pd_cur_date.strftime("%Y-%m-%d"),
            'illnesses': {'Милдью': mildew_prob,
